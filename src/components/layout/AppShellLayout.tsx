...
export default function AppShellLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#f2f2f2] text-slate-900">
      <div className="sticky top-0 z-30 border-b border-slate-200/80 bg-white backdrop-blur">
        <div className="mx-auto flex max-w-[1800px] items-center justify-between gap-3 px-3 py-3 sm:px-4 lg:px-5">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => navigate('/')} className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-sm sm:px-4">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#03cd8c] text-white shadow-lg shadow-sm/20"><Sparkles className="h-5 w-5" /></div>
          ...
          <div className="hidden min-w-0 flex-1 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm text-slate-500 shadow-sm md:flex">
      ...
      <div className="mx-auto flex max-w-[1800px] gap-5 px-3 pb-10 pt-4 sm:px-4 lg:px-5">
        ...
        <main className="min-w-0 flex-1">
          <Card className="fh-card mb-5 rounded-xl border border-slate-200 bg-white shadow-sm">
  ...
  return (
    <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
      {roles.map((item) => {
        ...
        return (
          <button key={item} onClick={() => onChange(item)} className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${active ? item === 'provider' ? 'bg-[#fff8ef] text-[#f77f00]' : item === 'admin' ? 'bg-slate-900 text-white' : 'bg-[#03cd8c] text-white' : 'text-slate-600 hover:bg-[#f8fafc]'}`}>
  ...
  return (
    <Card className="fh-card overflow-visible rounded-xl border border-slate-200 bg-white shadow-sm">
      <CardContent className="fh-card-content p-3">
        <div className="mb-3 rounded-lg border border-[#03cd8c]/20 bg-[#ecfff8] p-4">
            ...
            <div key={section} className="space-y-2">
              ...
              <div className="space-y-1">
                {items.map((item: any) => {
                  ...
                  return (
                    <button key={item.id} onClick={() => navigate(item.path)} className={`group flex w-full items-center gap-3 rounded-lg border px-3 py-3 text-left transition ${active ? 'border-[#03cd8c]/25 bg-[#ecfff8]' : 'border-transparent bg-white hover:border-slate-200 hover:bg-[#f8fafc]'}`}>
